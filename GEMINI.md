...
`../bayaryuk-frontend/docs/modules/<MODULE>.md`

The unified doc covers business rules, data model, API contract, backend implementation (this repo's `src/<module>/`), and frontend implementation. Structure follows `../bayaryuk-frontend/docs/modules/_TEMPLATE.md`.
...
1. **Before starting any work** on a module (e.g. anything under `src/<module>/`):
   - Look for `../bayaryuk-frontend/docs/modules/<MODULE>.md` (check `../bayaryuk-frontend/docs/modules/README.md` for the index).
   - If it exists → **read it first** to understand contract, business rules, edge cases.
   - If it does not exist → create it as part of the work using `../bayaryuk-frontend/docs/modules/_TEMPLATE.md`.
...
- API contract changes (new fields, endpoints, request/response shape) are captured in the unified module doc at `../bayaryuk-frontend/docs/modules/<MODULE>.md` (API Contract + Backend Implementation sections) — do NOT create a separate briefing file in this repo
- Always update the unified module doc after making changes (see "Module Documentation" section); if none exists, create one from `../bayaryuk-frontend/docs/modules/_TEMPLATE.md`

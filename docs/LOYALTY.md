...
Returns the program regardless of `isActive` status, so the frontend always stays in update mode once a program exists. Returns HTTP 200 with `null` body if no program has ever been created (not 404).

**Frontend logic:** If response is `null` → show create form. If response has data → always show update form, using `isActive` to reflect the toggle state.

### 3.2 POST `/loyalty/program` — Create

Creates a new loyalty program. Only one program per tenant allowed. `isActive` defaults to `true`.

**Side effect:** All existing customers are automatically enrolled into the program in the background (bulk enrollment). No manual enrollment calls needed.
...
Update program config. Partial updates supported. Same fields as create (all optional), plus `isActive` (boolean) to enable/disable the program.

**Deactivation** (`isActive: false`): Records `deactivatedAt` timestamp. All write operations (earn/redeem/adjust/enroll) are blocked. Subscription and point expiry timers are effectively paused.

**Re-activation** (`isActive: true`): Extends all active subscription `endDate` and point batch `expiresAt` by the number of inactive days. Bulk-enrolls any customers created during the inactive period. Clears `deactivatedAt`.

---

## 4. Customer Enrollment & Account

**Auto-enrollment:** Enrollment is automatic. New customers are enrolled on creation (`POST /customer`) if an active loyalty program exists. Existing customers are bulk-enrolled when a program is created or re-activated. No manual enrollment calls needed from the frontend.

### 4.1 POST `/loyalty/enroll`

Manual enrollment fallback. Enroll a customer in the loyalty program. Rejected when program is deactivated.
...
List all packages regardless of `isActive` status. Use the `isActive` field to display active/inactive badges in the UI. Inactive packages cannot be used for new subscriptions.
...
Partial update. Same fields as create, plus `isActive` (boolean) to activate/deactivate the package. Include `categoryIds` array to replace linked categories.

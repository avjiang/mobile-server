...
import service from "./expense-recurring-template.service"
import { ExpenseRecurringTemplate } from "../../prisma/client/generated/client"
import NetworkRequest from "../api-helpers/network-request"
import { RequestValidateError } from "../api-helpers/error"
import { sendResponse } from "../api-helpers/network"
import { AuthRequest } from "../middleware/auth-request"
import { CreateExpenseRecurringTemplateRequestBody, PostMonthRequestBody } from "./expense-recurring-template.request"
...
let getAll = (req: AuthRequest, res: Response, next: NextFunction) => {
    ...
    service.getAllTemplates(req.user.databaseName, syncRequest)
        .then(({ templates, total, serverTimestamp }) => sendResponse(res, { data: templates, total, serverTimestamp }))
        .catch(next);
}

let createMany = (req: NetworkRequest<CreateExpenseRecurringTemplateRequestBody>, res: Response, next: NextFunction) => {
    if (!req.user) throw new RequestValidateError('User not authenticated');
    if (Object.keys(req.body).length === 0) throw new RequestValidateError('Request body is empty')
    service.createMany(req.user.databaseName, req.body.templates)
        .then((count: number) => sendResponse(res, count))
        .catch(next)
}

let update = (req: NetworkRequest<ExpenseRecurringTemplate>, res: Response, next: NextFunction) => {
    if (!req.user) throw new RequestValidateError('User not authenticated');
...
let postMonth = (req: NetworkRequest<PostMonthRequestBody>, res: Response, next: NextFunction) => {
    if (!req.user) throw new RequestValidateError('User not authenticated');
    const periodMonth = req.body?.periodMonth;
    if (!periodMonth) throw new RequestValidateError('periodMonth is required')
    service.postMonth(req.user.databaseName, periodMonth)
        .then((count: number) => sendResponse(res, count))
        .catch(next)
}

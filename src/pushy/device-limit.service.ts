...
class DeviceLimitService {
  private static instance: DeviceLimitService;
  private globalPrisma: GlobalPrismaClient;
  private readonly BASE_DEVICE_LIMIT = 5;
  private readonly COST_PER_ADDITIONAL_DEVICE = 10000; // IDR 10,000 per month

  private constructor() {
    this.globalPrisma = getGlobalPrisma();
  }

  public static getInstance(): DeviceLimitService {
    if (!DeviceLimitService.instance) {
      DeviceLimitService.instance = new DeviceLimitService();
    }
    return DeviceLimitService.instance;
  }

  public async checkDeviceLimit(tenantId: number): Promise<DeviceLimitCheck> {
    try {
      // Get all active outlets for this tenant
      const tenantOutlets = await this.globalPrisma.tenantOutlet.findMany({
        where: {
          tenantId,
          isActive: true
        },
      ...
      for (const outlet of tenantOutlets) {
        for (const subscription of outlet.subscriptions) {
          ...
          const basePlanLimit = subscription.subscriptionPlan?.maxDevices || this.BASE_DEVICE_LIMIT;
      ...
      const currentDevices = await this.globalPrisma.pushyDeviceAllocation.count({
        where: {
          tenantId,
          device: {
            isActive: true
      ...
      return {
        ...
        maxAllowed,
        requiresPayment,
        additionalCost: requiresPayment ? this.COST_PER_ADDITIONAL_DEVICE : undefined,
        message: canAddDevice
          ? 'Device can be added'
          : requiresPayment
            ? `Device limit reached. Purchase additional device slot for IDR ${this.COST_PER_ADDITIONAL_DEVICE.toLocaleString('id-ID')}/month`
  ...
  public async allocateDevice(
    ...
    try {
      ...
      const subscription = await this.globalPrisma.tenantSubscription.findFirst({
        where: {
          tenantId,
          status: 'Active'
        }
      ...
      const allocation = await this.globalPrisma.pushyDeviceAllocation.create({
        data: {
          tenantId,
          deviceId,
  ...
  public async deallocateDevice(deviceId: number): Promise<void> {
    try {
      await this.globalPrisma.pushyDeviceAllocation.delete({
        where: {
          deviceId
        }
      });
  ...
  public async getTenantDeviceStats(tenantId: number): Promise<any> {
    try {
      // Get all devices allocated to this tenant
      const allocations = await this.globalPrisma.pushyDeviceAllocation.findMany({
        where: {
          tenantId
        },
        include: {
  ...
  public async purchaseAdditionalDevice(
    tenantId: number,
    quantity: number = 1,
    outletId?: number
  ): Promise<any> {
    try {
      ...
      if (outletId) {
        subscription = await this.globalPrisma.tenantSubscription.findFirst({
          where: {
            tenantId,
            outletId,
            status: {
      ...
      let addOn = await this.globalPrisma.subscriptionAddOn.findFirst({
        where: {
      ...
      if (!addOn) {
        addOn = await this.globalPrisma.subscriptionAddOn.create({
          data: {
            name: 'Push Notification Device',
            addOnType: 'device',
            pricePerUnit: this.COST_PER_ADDITIONAL_DEVICE,
      ...
      const existingAddOn = await this.globalPrisma.tenantSubscriptionAddOn.findFirst({
        where: {
      ...
      if (existingAddOn) {
        // Update quantity
        const updated = await this.globalPrisma.tenantSubscriptionAddOn.update({
          where: {
            id: existingAddOn.id
          },
          data: {
        ...
        return {
          ...
          monthlyAdditionalCost: updated.quantity * this.COST_PER_ADDITIONAL_DEVICE
        };
      } else {
        // Create new add-on usage
        const created = await this.globalPrisma.tenantSubscriptionAddOn.create({
          data: {
            tenantSubscriptionId: subscription.id,
            addOnId: addOn.id,
            quantity
        ...
        return {
          ...
          monthlyAdditionalCost: created.quantity * this.COST_PER_ADDITIONAL_DEVICE
        };
      }
    } catch (error) {
  ...
  public async removeAdditionalDevice(
    tenantId: number,
    quantity: number = 1
  ): Promise<any> {
    try {
      ...
      if (deviceAddOn.quantity <= quantity) {
        ...
        await this.globalPrisma.tenantSubscriptionAddOn.delete({
          where: {
            id: deviceAddOn.id
          }
        });
...
export default DeviceLimitService.getInstance();